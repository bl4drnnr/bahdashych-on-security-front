...
const DropDown = ({ items, value }: IDropDown) => {
          ...
          <ul className=" dropdown-menu min-w-max absolute bg-white text-base z-50 float-left py-2 list-none text-left rounded-lg shadow-lg mt-1 m-0 bg-clip-padding border-none">
            {items.map(item => (
              <li
                key={item}
              >
                <a className={'cursor-pointer dropdown-item text-sm py-2 px-4 font-normal block w-full whitespace-nowrap bg-transparent text-gray-700 hover:bg-gray-100'}
              >{item}</a>
              </li>
            ))}
          </ul>
